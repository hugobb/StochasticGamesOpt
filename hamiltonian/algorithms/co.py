...
class ConsensusOptimization(Algorithm):
    ...
    def __init__(self, game, lr_H=None, batch_size=1, full_batch=False, *args, **kwargs):
    ...
    def update(self):
        ...
        for i, player in enumerate(self.game.get_players()):
            for p, g1, g2, gH in zip(player.parameters(), grad_1[i], grad_2[i], grad_H[i]):
...
class SVRCO(ConsensusOptimization):
    ...
    def update(self):
        if self.full_batch:
            ...
            x_1, x_2 = self.sampler.sample(self.batch_size)
            grad_1, grad_2 = self.game.grad(x_1), self.game.grad(x_2)
            grad_snapshot_1, grad_snapshot_2 = self.snapshot.grad(x_1), self.snapshot.grad(x_2)
            self.n_samples += 2*self.batch_size
        ...
        for i, player in enumerate(self.game.get_players()):
            for p, g1, g2, gs1, gs2, bg, g_H, gs_H, bg_H in zip(player.parameters(), grad_1[i], grad_2[i], grad_snapshot_1[i], grad_snapshot_2[i],
                                                                 self.full_grad[i], grad_H[i], snapshot_grad_H[i], self.full_grad_H[i]):
