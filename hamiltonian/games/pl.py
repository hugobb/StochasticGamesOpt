...
class PL2dGame(Game):
    ...
    def loss(self, x=None):
        loss = (self.players[0][0]**2 + 3*torch.sin(self.players[0][0])**2*torch.sin(self.players[1][0])**2 - 4*self.players[1][0]**2 - 10*torch.sin(self.players[1][0])**2).sum()
        return [loss, -loss]
...
class RobustLeastSquares(Game):
    ...
    def loss(self, x=None):
        loss = (self.A.mv(self.players[0][0]) - self.players[1][0])**2 - self.lambda_reg*(self.players[1][0] - self.y_0)**2
        if x is None:
            loss = loss.mean()
        else:
            x = x.view(-1)
