...
class StochasticBilinearGame(Game):
    def __init__(self, dim, matrix=None, bias=True):
        ...
        self.a = (1./math.sqrt(self.dim) *
                  torch.zeros(n_samples, self.dim).normal_())
    ...
    def init(self):
        self.players[0][0].data = 1./math.sqrt(self.dim) * self.players[0][0].data.normal_()*10
        self.players[1][0].data = 1./math.sqrt(self.dim) * self.players[1][0].data.normal_()*10
    ...
    def loss(self, x=None):
        ...
        loss = (self.players[0][0].view(1, -1)*(self.matrix[x] *
                  self.players[1][0].view(1, 1, -1)).sum(-1) +
                  self.a[x] * self.players[0][0].view(1, -1) +
                  self.b[x] * self.players[1][0].view(1, -1)).sum(-1).mean()

        return [loss, -loss]

    def dist2opt(self):
        d = (((self.players[0][0].data.cpu() - self.x_star)**2).sum() + ((self.players[1][0].data.cpu() - self.y_star)**2).sum())
        return d
