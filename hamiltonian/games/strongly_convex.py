...
from .utils import make_commutative_matrix, make_random_matrix, make_spd_matrix, make_sym_matrix
...
class ScPLplusGame(Game):
    def __init__(self):
        ...
        self.players = nn.ModuleList([nn.ParameterList([nn.Parameter(torch.zeros(self.dim))])])
...
class QuadraticOptimization(Game):
    ...
    def init(self):
        self.players[0][0].data = 1./math.sqrt(self.dim) * torch.zeros(self.dim).normal_()

    def loss(self, x=None):
        if x is None:
            x = self.sampler.sample_batch()
        
        loss = (self.players[0][0].view(1, -1)*(self.matrix[x] *
                  self.players[0][0].view(1, 1, -1)).sum(-1)
                  ).sum(-1).mean()
...
class QuadraticGame(Game):
    def __init__(self, dim, num_samples=1, mu=0.0, L=1.0, mu_B=0.0, L_B=1.0, num_zeros=0, A=None, B=None, C=None, bias=False, normal=True, init_func=None):
        super().__init__()
        ...
        if B is None:
            self.matrix[2] = make_random_matrix(num_samples, self.dim, mu_B, L_B, normal=normal)
        else:
            self.matrix[2] = B

        self.bias = torch.zeros(3, num_samples, dim)
        if bias:
            self.bias = 1./math.sqrt(self.dim) * self.bias.normal_()/10
        ...
        for i in range(num_samples):
            for j in range(num_samples):
                J_ij[i, j] = (torch.mm(J[i].transpose(-1, -2), J[j]) +  torch.mm(J[j].transpose(-1, -2), J[i]))/2
        ...
        self.ell = float((1/((1/e).real.min(1))).max())
        self.ell_mean = float(1/((1/e_mean).real.min()))
        self.mu = float(e.real.min())
        self.L_H = float(s_mean.max().item())
        self.L_max = float(s.max().item())
        self.mu_H = float(s_mean.min().item())
    ...
    def reset(self):
        if self.init_func is None:
            self.players[0][0].data = 1./math.sqrt(self.dim) * self.players[0][0].data.normal_()
            self.players[1][0].data = 1./math.sqrt(self.dim) * self.players[1][0].data.normal_()
        else:
    ...
    def loss(self, x=None):
        ...
        loss = (self.players[0][0].view(1, -1)*(self.matrix[0, x]*self.players[0][0].view(1, 1, -1)).sum(-1)/2 + 
                self.players[0][0].view(1, -1)*(self.matrix[2, x]*self.players[1][0].view(1, 1, -1)).sum(-1) -
                self.players[1][0].view(1, -1)*(self.matrix[1, x]*self.players[1][0].view(1, 1, -1)).sum(-1)/2 +
                self.players[0][0].view(1, -1)*self.bias[0, x] - self.players[1][0].view(1, -1)*self.bias[1, x] #+ self.bias[2, x]
        ).sum(-1).mean()

        return [loss, -loss]

    def dist2opt(self):
        d = ((self.players[0][0].data.detach().cpu() - self.x_star)**2).sum() + ((self.players[1][0].data.detach().cpu() - self.y_star)**2).sum()
        return d
...
class L2RegressionSP(Game):
    ...
    def init(self):
        self.players[0][0].data = 1./math.sqrt(self.dim) * torch.zeros(self.dim).normal_()
        self.players[1][0].data = 1./math.sqrt(self.dim) * torch.zeros(self.dim).normal_()

    def loss(self, x=None):
        if x is None:
            x = self.sampler.sample_batch()
        
        loss = (0.5*self.reg*(self.players[0][0]**2).sum(-1) + 
                1/self.n*(- 0.5*(self.players[1][0]**2).sum(-1) - (self.b*self.players[1][0]).sum(-1) +
    ...
    def dist2opt(self):
        if self.x_star is None or self.y_star is None:
            self.x_star, self.y_star = torch.zeros_like(self.players[0][0]), torch.zeros_like(self.players[1][0])
        d = ((self.players[0][0].data.detach() - self.x_star)**2).sum() + ((self.players[1][0].data.detach() - self.y_star)**2).sum()
        return d
...
class RidgeRegressionSP(Game):
    ...
    def init(self):
        self.players[0][0].data = 1./math.sqrt(self.dim) * torch.zeros(self.dim).normal_()
        self.players[1][0].data = 1./math.sqrt(self.num_samples) * torch.zeros(self.num_samples).normal_()

    def loss(self, x=None):
        if x is None:
            loss = (self.matrix.mv(self.players[0][0]).dot(self.players[1][0])/self.num_samples 
                    + (self.players[0][0]**2).sum()/2 
                    - (self.players[1][0]**2).sum()/2
                    )
        else:
            loss = (self.matrix[x].mv(self.players[0][0])*self.players[1][0][x] 
    ...
    def dist2opt(self):
        if self.x_star is None or self.y_star is None:
            self.x_star, self.y_star = self.optimum()
        d = (((self.players[0][0] - self.x_star)**2).sum() +
             ((self.players[1][0] - self.y_star)**2).sum())
        return d
        

    def optimum(self):
        return torch.zeros_like(self.players[0][0]), torch.zeros_like(self.players[1][0])
